...
import numpy as np
from utils import np_array_to_proto_matrix, vector_to_proto_vector, pickle_string_to_obj, obj_to_pickle_string
...
def test_get_init_model():
    proto_request = api_pb2.ModelUpdateRequest(training_data_x=x_matrix, training_data_y=y_vector, device_type=device_type, model_type=model_type)
...
def test_get_updted_model():
    ...
    proto_request = api_pb2.ModelUpdateRequest(training_data_x=x_matrix, training_data_y=y_vector, device_type=device_type, old_model=old_model)
    stub_ = api_pb2_grpc.CloudServiceStub(channel_cloud)
