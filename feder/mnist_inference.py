...
def inference(images, Hidden1, Hidden2):
    ...
    with tf.name_scope("hidden1"):
        ...
        weights = tf.Variable(
            init_weights([IMAGE_PIXELS, Hidden1]), name="weights", dtype=tf.float32
        )
    ...
    with tf.name_scope("hidden2"):
        ...
        weights = tf.Variable(
            init_weights([Hidden1, Hidden2]), name="weights", dtype=tf.float32
        )
    ...
    with tf.name_scope("out"):
        ...
        weights = tf.Variable(
            init_weights([Hidden2, NUM_CLASSES]), name="weights", dtype=tf.float32
        )
...
def placeholder_inputs(batch_size):
    ...
    images_placeholder = tf.placeholder(
        tf.float32, shape=(None, IMAGE_PIXELS), name="images_placeholder"
    )
...
def word_fully_connected_model(batch_size, hidden, window, num_classes, num_input):

    timesteps = window
    num_hidden = hidden

    data_placeholder = tf.placeholder("float", [None, timesteps, num_input])
    labels_placeholder = tf.placeholder("float", [None, num_classes])

    weights = {"out": tf.Variable(tf.random_normal([num_hidden, num_classes]))}
    biases = {"out": tf.Variable(tf.random_normal([num_classes]))}

    logits = RNN(data_placeholder, weights, biases, timesteps, hidden)
