...
def run_differentially_private_federated_averaging(
    ...
    for r in xrange(FLAGS.max_comm_rounds):
        ...
        if not (FLAGS.loaded and r == 0):
            ...
            # Y_batch_encoded = []
            # X_batch = np.array(data.x_vali)
            # X_batch = X_batch.reshape(b, window, num_input)
            # Y_batch_encoded = np.array(Y_batch_encoded)
            # Y_batch_encoded = Y_batch_encoded.reshape(b, num_classes)
            # batch_size=b
            # last_batch = len(data.y_vali) % batch_size
            # last_batch=len(data.y_vali)
            # batch_size=len(data.y_vali)
        ...
        if delta_accountant[-1] > privacy_agent.get_bound() or math.isnan(
            ...
            print(
                "************** The last step exhausted the privacy budget **************"
            )
        ...
        for c in range(m):
            ...
            for e in xrange(int(FLAGS.e)):
                for step in xrange(len(data_ind)):
                    ...
                    for x in Y_batch:
                        on_hot_vector = np.zeros([num_classes], dtype=float)
                        on_hot_vector[x] = 1.0
                        Y_batch_encoded = np.concatenate(
                            (Y_batch_encoded, on_hot_vector)
                        )
            ...
            if c == 0:
                ...
                weights_accountant_prepare = WeightsAccountant(
                    sess, model, sigma, real_round
                )
            else:
                    ...
                    if r > 0:
                        if use_signi:
                            Significance = weights_accountant.checkSignificance(
                                sess, prev_Sanitized_Updates, threshold
                            )
                            if Significance:
