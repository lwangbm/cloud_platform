...
class AmortizedAccountant(object):
    ...
    def __init__(self, total_examples):
        ...
        self._eps_squared_sum = tf.Variable(
            tf.zeros([1]), trainable=False, name="eps_squared_sum"
        )
    ...
    def accumulate_privacy_spending(self, eps_delta, unused_sigma, num_examples):
        ...
        with tf.control_dependencies(
            ...
            amortize_ratio = (
                tf.cast(num_examples, tf.float32) * 1.0 / self._total_examples
            )
            ...
            # and proof.
            amortize_eps = tf.reshape(
                tf.log(1.0 + amortize_ratio * (tf.exp(eps) - 1.0)), [1]
            )
...
class GaussianMomentsAccountant(MomentsAccountant):
    ...
    def _differential_moments(self, sigma, s, t):
        ...
        ), "The order of %d is out " "of the upper bound %d." % (
            t,
        ...
        exponents = tf.constant(
            [j * (j + 1.0 - 2.0 * s) / (2.0 * sigma * sigma) for j in range(t + 1)],
            dtype=tf.float64,
        )
    ...
    def _compute_log_moment(self, sigma, q, moment_order):
        ...
        binomial_table = tf.slice(
            self._binomial_table, [moment_order, 0], [1, moment_order + 1]
        )
